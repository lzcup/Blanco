...
namespace Blanco
{
	...
	void Renderer2D::DrawQuad(const glm::vec2& position, float rotation, const glm::vec2& size, const Ref<Texture2D>& texture, const glm::vec4& color, int repeats)
	{
		DrawQuad(glm::vec3(position, 0.0f), rotation, size, texture, color, repeats);
	}
	void Renderer2D::DrawQuad(const glm::vec3& position, float rotation, const glm::vec2& size, const Ref<Texture2D>& texture, const glm::vec4& color, int repeats)
	{
		s_Data->TextureShader->Bind();
		glm::mat4 transform =
			glm::translate(glm::mat4(1.0f), position) *
			...
			glm::scale(glm::mat4(1.0f), glm::vec3(size, 1.0f));
		s_Data->TextureShader->SetMat4("u_Transform", transform);
		s_Data->TextureShader->SetInt("u_Repeats", repeats);
		s_Data->TextureShader->SetFloat4("u_TextureColor", color);
